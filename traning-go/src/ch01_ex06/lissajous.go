...
func lissajous(out io.Writer) {
	...
	for i := 0; i < nframes; i++ {
		...
		for t := 0.0; t < cycles*2*math.Pi; t += res {
			...
			//img.SetColorIndex(size+int(x*size+0.5), size+int(y*size+0.5), uint8(rand.Int()))
			//img.SetColorIndex(size+int(x*size+0.5), size+int(y*size+0.5), uint8(2) * 0x11)
	...
	//ファイル出力
	f, _ := os.OpenFile("data/lissajous_ex6.gif", os.O_WRONLY|os.O_CREATE, 0600)
	defer f.Close()
